...
var pathtests = []struct {
	path	string
	isValid	bool
}{{".good", true}, {"./good", true}, {"/good", true}, {"../good", true}, {"bad", false}}
...
func TestOverwrittenFile(t *testing.T) {
	...
	files := []ignv2_2types.File{{Node: ignv2_2types.Node{Path: "fixtures/test1.txt"}, FileEmbedded1: ignv2_2types.FileEmbedded1{Contents: ignv2_2types.FileContents{Source: dataurl.EncodeBytes([]byte("hello world\n"))}, Mode: &fileMode}}}
	if status := checkFiles(files); !status {
		t.Errorf("Invalid files")
	}
	files = []ignv2_2types.File{{Node: ignv2_2types.Node{Path: "fixtures/test1.txt"}, FileEmbedded1: ignv2_2types.FileEmbedded1{Contents: ignv2_2types.FileContents{Source: dataurl.EncodeBytes([]byte("hello\n"))}, Mode: &fileMode}}, {Node: ignv2_2types.Node{Path: "fixtures/test1.txt"}, FileEmbedded1: ignv2_2types.FileEmbedded1{Contents: ignv2_2types.FileContents{Source: dataurl.EncodeBytes([]byte("hello world\n"))}, Mode: &fileMode}}}
	if status := checkFiles(files); !status {
...
func TestDaemonOnceFromNoPanic(t *testing.T) {
	...
	defer close(stopCh)
	dn, err := New("/", "testnodename", "testos", NewNodeUpdaterClient(), "", "test", false, nil, k8sfake.NewSimpleClientset(), false, "", nil, exitCh, stopCh)
	require.Nil(t, err)
	require.NotPanics(t, func() {
		dn.triggerUpdateWithMachineConfig(&mcfgv1.MachineConfig{}, &mcfgv1.MachineConfig{})
	})
}
...
func (f *fixture) newController() *Daemon {
	...
	d, err := NewClusterDrivenDaemon("/", "node_name_test", "rhel", NewNodeUpdaterClient(), i.Machineconfiguration().V1().MachineConfigs(), f.kubeclient, "", "", false, k8sI.Core().V1().Nodes(), false, "", newFakeNodeWriter(), nil, nil)
	if err != nil {
...
func TestPrepUpdateFromClusterOnDiskDrift(t *testing.T) {
	...
	annotations := map[string]string{constants.CurrentMachineConfigAnnotationKey: "test1", constants.DesiredMachineConfigAnnotationKey: "test2", constants.MachineConfigDaemonStateAnnotationKey: ""}
	...
	f = newFixture(t)
	onDiskMC = newMachineConfig("test1")
	annotations = map[string]string{constants.CurrentMachineConfigAnnotationKey: "test1", constants.DesiredMachineConfigAnnotationKey: "test1", constants.MachineConfigDaemonStateAnnotationKey: constants.MachineConfigDaemonStateDone}
	node = newNode(annotations)
	f.objects = append(f.objects, newMachineConfig("test1"))
	f.objects = append(f.objects, newMachineConfig("test2"))
	tmpCurrentConfig.Truncate(0)
