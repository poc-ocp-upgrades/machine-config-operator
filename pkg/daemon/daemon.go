...
func New(rootMount string, nodeName string, operatingSystem string, nodeUpdaterClient NodeUpdaterClient, onceFrom string, skipReboot bool, mcClient mcfgclientset.Interface, kubeClient kubernetes.Interface, kubeletHealthzEnabled bool, kubeletHealthzEndpoint string, nodeWriter *NodeWriter, exitCh chan<- error, stopCh <-chan struct{}) (*Daemon, error) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	var (
		osImageURL	string
	...
	dn := &Daemon{name: nodeName, OperatingSystem: operatingSystem, NodeUpdaterClient: nodeUpdaterClient, rootMount: rootMount, bootID: bootID, bootedOSImageURL: osImageURL, onceFrom: onceFrom, skipReboot: skipReboot, kubeletHealthzEnabled: kubeletHealthzEnabled, kubeletHealthzEndpoint: kubeletHealthzEndpoint, nodeWriter: nodeWriter, exitCh: exitCh, stopCh: stopCh, kubeClient: kubeClient, mcClient: mcClient}
	dn.atomicSSHKeysWriter = dn.atomicallyWriteSSHKey
	return dn, nil
}
func NewClusterDrivenDaemon(rootMount, nodeName, operatingSystem string, nodeUpdaterClient NodeUpdaterClient, mcInformer mcfginformersv1.MachineConfigInformer, kubeClient kubernetes.Interface, onceFrom string, skipReboot bool, nodeInformer coreinformersv1.NodeInformer, kubeletHealthzEnabled bool, kubeletHealthzEndpoint string, nodeWriter *NodeWriter, exitCh chan<- error, stopCh <-chan struct{}) (*Daemon, error) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	dn, err := New(rootMount, nodeName, operatingSystem, nodeUpdaterClient, onceFrom, skipReboot, nil, kubeClient, kubeletHealthzEnabled, kubeletHealthzEndpoint, nodeWriter, exitCh, stopCh)
	if err != nil {
		return nil, err
	}
