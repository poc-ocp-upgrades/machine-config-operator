...
type NodeWriter struct{ writer chan message }

func NewNodeWriter() *NodeWriter {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &NodeWriter{writer: make(chan message, defaultWriterQueue)}
}
...
func (nw *NodeWriter) SetDone(client corev1.NodeInterface, lister corelisterv1.NodeLister, node string, dcAnnotation string) error {
	_logClusterCodePath()
	defer _logClusterCodePath()
	annos := map[string]string{constants.MachineConfigDaemonStateAnnotationKey: constants.MachineConfigDaemonStateDone, constants.CurrentMachineConfigAnnotationKey: dcAnnotation}
...
func (nw *NodeWriter) SetUnreconcilable(err error, client corev1.NodeInterface, lister corelisterv1.NodeLister, node string) error {
	...
	annos := map[string]string{constants.MachineConfigDaemonStateAnnotationKey: constants.MachineConfigDaemonStateUnreconcilable}
	respChan := make(chan error, 1)
	nw.writer <- message{client: client, lister: lister, node: node, annos: annos, responseChannel: respChan}
	clientErr := <-respChan
	if clientErr != nil {
...
func (nw *NodeWriter) SetDegraded(err error, client corev1.NodeInterface, lister corelisterv1.NodeLister, node string) error {
	...
	annos := map[string]string{constants.MachineConfigDaemonStateAnnotationKey: constants.MachineConfigDaemonStateDegraded}
	respChan := make(chan error, 1)
	nw.writer <- message{client: client, lister: lister, node: node, annos: annos, responseChannel: respChan}
	clientErr := <-respChan
	if clientErr != nil {
