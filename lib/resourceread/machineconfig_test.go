...
func TestReadMachineConfig(t *testing.T) {
	...
	}{{desc: "test valid machine config", wantError: false, mc: &mcfgv1.MachineConfig{metav1.TypeMeta{Kind: testMCKind, APIVersion: testMCVer}, metav1.ObjectMeta{Name: testMCName}, mcfgv1.MachineConfigSpec{}}}, {desc: "test nil machine config", wantError: true}, {desc: "test invalid machine config", wantError: true, mc: &mcfgv1.MachineConfig{metav1.TypeMeta{Kind: "invalidMachineConfig", APIVersion: "invalidAPIVersion"}, metav1.ObjectMeta{Name: "test invalid machine config"}, mcfgv1.MachineConfigSpec{}}}}
	for _, tc := range tcs {
		var b []byte
		if tc.mc != nil {
